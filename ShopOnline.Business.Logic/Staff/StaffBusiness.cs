...
namespace ShopOnline.Business.Logic.Staff
{
    public class StaffBusiness : IStaffBusiness
    {
        ...
        public async Task<IPagedList<StaffInfor>> GetListStaffAccAsync(string sortOrder, TypeAcc typeAcc, string searchString, int? page)
        {
            var listStaff = new List<StaffInfor>();
            var staffs = await _context.Staffs.Where(x => !x.IsDeleted && x.TypeAcc == typeAcc).ToListAsync();
            if (staffs != null)
            {
        ...
        public async Task CreateAsync(StaffCreate staffCreate, TypeAcc typeAcc)
        {
        ...
        public StaffEdit GetStaffById(int id, TypeAcc typeAcc)
        {
            var staff = _context.Staffs.Where(x => x.Id == id && !x.IsDeleted && x.TypeAcc == typeAcc).Select(x => new StaffEdit
            {
                Id = x.Id,
                FullName = x.FullName,
                Email = x.Email,
