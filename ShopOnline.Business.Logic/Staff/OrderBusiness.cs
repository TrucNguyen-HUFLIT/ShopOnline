...
namespace ShopOnline.Business.Logic.Staff
{
    public class OrderBusiness : IOrderBusiness
    {
        ...
        {
            var productOrderDetails = await _context.OrderDetails.Where(x => x.IdOrder == orderId).Select(x => new { x.ProductDetail, x.QuantityPurchased }).ToListAsync();
            foreach (var productOrderDetail in productOrderDetails)
            {
                productOrderDetail.ProductDetail.Quantity += productOrderDetail.QuantityPurchased;
            }
            _context.UpdateRange(productOrderDetails.Select(x => x.ProductDetail).ToArray());
            await _context.SaveChangesAsync();
        }
    }
}
